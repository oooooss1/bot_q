...
Token = "5488760420:AAFHjG3CNjDN0EVJaq16idF2jC2u2g3E6SM"

bot = TeleBot(Token)

user_ch = ("ieiyi") #معرف قناة النشر بدون @

key , dev = Mk() , Mk() 

dev.add(btn(text=" • المطور • ",url="t.me/devnoxi") )

key.add(btn(text="• تفعيل النشر التلقائي 🔛 •",callback_data="run"))

key.add(btn(text="• قناة النشر التلقائي  •", callback_data="ch")) 

key.add(btn(text=" • المطور • ",url="t.me/devnoxi"))
...
def start(message):
	...
	tag = f"[{message.from_user.first_name}](tg://settings)"	text = f'''*أهلا بك عزيزي •* {tag} *• 
